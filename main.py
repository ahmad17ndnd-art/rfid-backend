...
if DATABASE_URL.startswith("postgres://"):
    # Render (وبعض المزودين) بيعطوا رابط بادئته postgres:// وpsycopg2 بيفضل postgresql://
...
def init_db():
    ...
    """)
    c.execute("SELECT COUNT(*) as cnt FROM door_commands")
    if c.fetchone()["cnt"] == 0:
        c.execute("INSERT INTO door_commands (id, pending) VALUES (1, 0)")
    ...
    """)
    c.execute("INSERT INTO notification_settings (id) VALUES (1) ON CONFLICT (id) DO NOTHING")

    c.execute("ALTER TABLE cards ADD COLUMN IF NOT EXISTS is_emergency INTEGER DEFAULT 0")
    c.execute("ALTER TABLE logs ADD COLUMN IF NOT EXISTS offline INTEGER DEFAULT 0")
...
def auth_google(req: GoogleLoginRequest):
    ...
    c.execute(
        "INSERT INTO admins (email, google_sub, created_at) VALUES (%s, %s, %s) RETURNING id",
...
def update_card(card_id: int, req: CardUpdateRequest, admin=Depends(get_current_admin)):
    ...
    if updates:
        ...
        c.execute(f"UPDATE cards SET {fields} WHERE id = %s", values)
        conn.commit()
    conn.close()
    return {"status": "ok"}
...
def request_door_open(admin=Depends(get_current_admin)):
    conn = get_db()
    c = conn.cursor()
    c.execute("UPDATE door_commands SET pending = 1, requested_at = %s WHERE id = 1",
...
def device_scan(req: ScanRequest, ok=Depends(verify_device_key)):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT * FROM cards WHERE uid = %s", (req.uid,))
    card = c.fetchone()
    ...
    if not card:
        c.execute(
            "INSERT INTO logs (card_uid, person_name, event_type, granted, reason, timestamp) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    c.execute(
        "SELECT event_type FROM logs WHERE card_uid = %s AND granted = 1 ORDER BY id DESC LIMIT 1",
        (req.uid,)
    )
    last = c.fetchone()
    ...
    for entry in req.entries:
        c.execute(
            """INSERT INTO logs (card_uid, person_name, event_type, granted, reason, offline, timestamp)
               VALUES (%s, %s, %s, 1, 'emergency_offline', 1, %s)""",
